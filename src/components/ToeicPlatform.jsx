...
function ToeicPlatform() {
  ...
  const handleEnter = () => {
    ...
    fetchStudent(id)
      .then((student) => {
        ...
        setStudentPoints(Number(student.points) ?? 0);
        setStudentBadges(Array.isArray(student.badges) ? student.badges : []);
  ...
  useEffect(() => {
    if (activeSection !== 'vocabulary' || !studentId) return;
    setVocabularyLoading(true);
    setVocabularyError('');
    fetchVocabulary(studentId, 300)
      .then((list) => setVocabularyList(Array.isArray(list) ? list : []))
      .catch((err) => {
        setVocabularyError(err?.message || '단어장을 불러오지 못했어요.');
        setVocabularyList([]);
      })
  ...
  const handleAddVocabulary = async () => {
    ...
    if (!word || vocabSaving || !studentId) return;
    setVocabSaving(true);
    setVocabularyError('');
    try {
      const item = await addVocabulary(studentId, { word, meaning: vocabMeaning.trim() });
      setVocabularyList((prev) => [{ id: item.id, word: item.word, meaning: item.meaning, createdAt: item.createdAt }, ...prev]);
      setVocabWord('');
      setVocabMeaning('');
    } catch (err) {
      setVocabularyError(err?.message || '단어 추가에 실패했어요.');
    } finally {
  ...
  const handleDeleteVocabulary = async (id) => {
    if (!studentId) return;
    try {
      await deleteVocabulary(studentId, id);
      setVocabularyList((prev) => prev.filter((item) => item.id !== id));
    } catch (err) {
      setVocabularyError(err?.message || '삭제에 실패했어요.');
    }
  };

  const applySaveRewards = (res) => {
    if (res?.totalPoints != null) setStudentPoints(Number(res.totalPoints));
    if (res?.badgesEarned?.length) {
      setStudentBadges((prev) => {
        const added = res.badgesEarned.filter(
          (key) => !prev.some((b) => (b.badge_key || b) === key)
        );
        return [
          ...prev,
          ...added.map((badge_key) => ({ badge_key, earned_at: new Date().toISOString() })),
        ];
      });
    }
  };
    ...
    <div className="toeic-platform">
          ...
          <div className="hero-rewards">
            ...
            {studentBadges.length > 0 && (
              <div className="reward-badges">
                {studentBadges.map((b) => (
                  <span key={b.badge_key || b} className="reward-badge" title={BADGE_LABELS[b.badge_key || b] || b.badge_key || b}>
                    {BADGE_LABELS[b.badge_key || b] || b.badge_key || b}
                  </span>
                ))}
              </div>
            )}
      ...
      {activeSection === 'ranking' && (
        <section id="section-ranking" className="platform-section">
          <div className="section-header">
            <h2>학원 랭킹</h2>
            <p>학습 포인트 순위입니다. 활동할수록 포인트가 쌓여요.</p>
          </div>
          <div className="card">
            {leaderboardLoading && <p className="empty-text">랭킹 불러오는 중...</p>}
            {leaderboardError && <p className="error-text">{leaderboardError}</p>}
            {!leaderboardLoading && !leaderboardError && leaderboard.length === 0 && (
              <p className="empty-text">아직 랭킹 데이터가 없어요.</p>
            )}
            {!leaderboardLoading && !leaderboardError && leaderboard.length > 0 && (
                ...
                <table className="leaderboard-table">
                  ...
                  <tbody>
                    {leaderboard.map((row) => (
                      <tr
                        key={row.studentId}
                        className={row.studentId === studentId ? 'current-user' : ''}
                      >
                        <td>{row.rank}</td>
                        <td>{row.name}</td>
                        <td>{row.points}P</td>
      ...
      {activeView === 'student' && activeSection === 'vocabulary' && (
        <section id="section-vocabulary" className="platform-section">
          <div className="section-header">
            <h2>내 단어장</h2>
            <p>학원에서 배운 단어를 저장하세요. 이동 중이나 집에서 모바일로 게임처럼 복습할 수 있어요.</p>
          </div>
          <div className="card vocabulary-card">
            {!studentId ? (
              <p className="empty-text">로그인하면 단어장을 사용할 수 있어요.</p>
            ) : (
              <>
                <div className="vocabulary-add">
                  ...
                  <button
                    ...
                    disabled={vocabSaving || !vocabWord.trim()}
                  >
                    {vocabSaving ? '추가 중...' : '추가'}
                  </button>
                </div>
                ...
                {!vocabularyLoading && vocabularyList.length === 0 && !vocabularyError && (
                  <p className="empty-text">아직 저장한 단어가 없어요. 위에서 단어를 추가해보세요.</p>
                )}
                {!vocabularyLoading && vocabularyList.length > 0 && (
